...
public class TopicOperator {
    ...
    Future reconcileAllTopics(String reconciliationType) {
        ...
        return listFut.<ReconcileState>compose(topicNamesFromKafka ->
            ...
            for (KafkaTopic kt : reconcileState.ktList) {
                ...
                if (reconcileState.failed.contains(topicName)) {
                    ...
                    // Reconcile
                    futs.add(dog(reconciliationType, kt, topic));
                    // TODO if success then remove from undetermined add to success
                } else {
                    // Topic exists in kube, but not in Kafka
                    futs.add(dog(reconciliationType, kt, topic));
                    // TODO if sucess then add to success
                }
            }
            // TODO finally anything left in undetermined doesn't exist in topic store not kube -> delete it in kafka
    ...
    /**
     * Reconcile all the topics found in Kafka, returning
     * @param reconciliationType
     * @param topicsFromKafka
     * @return
     */
    private Future<ReconcileState> reconcileFromKafka(String reconciliationType, List<TopicName> topicsFromKafka) {
        ...
        for (TopicName topicName : topicsFromKafka) {
            ...
            topicStore.read(topicName, tsr -> {
                if (tsr.failed()) {
                    topicFutures.add(Future.failedFuture(tsr.cause()));
                } else if (tsr.result() == null) {
                    topicFutures.add(Future.succeededFuture(Boolean.FALSE));
                } else {
                    topicFutures.add(cat(reconciliationType, topicName, tsr.result()));
                }
            });
        }
    ...
    private Future<Boolean> cat(String reconciliationType, TopicName topicName, Topic privateTopic) {
    ...
    /*
    private Future reconcileFromKube(String reconciliationType, ReconcileState reconcileState) {
        ...
        k8s.listMaps(resourcesListResult -> {
            ...
            if (resourcesListResult.succeeded()) {
                ...
                Map<String, KafkaTopic> resourcesMap = resources.stream().collect(Collectors.toMap(
                    ...
                    resource -> resource));
                resourcesMap.keySet().removeAll(reconcileState.kafkaTopics);
                LOGGER.debug("Reconciling KafkaTopics: {}", resourcesMap.keySet());
                for (KafkaTopic resource : resourcesMap.values()) {
                    Topic k8sTopic = resource != null ? TopicSerialization.fromTopicResource(resource) : null;
                    resourceFutures.add(dog(reconciliationType, resource, k8sTopic));
                }
    ...
    private Future<Void> dog(String reconciliationType, KafkaTopic resource, Topic k8sTopic) {
